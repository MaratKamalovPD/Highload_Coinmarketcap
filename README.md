...
![VWAP formula](https://latex.codecogs.com/png.latex?%5Ctext%7BVWAP%7D%20%3D%20%5Cfrac%7B%5Csum%20%5Ctext%7BPrice%7D_i%20%5Ctimes%20%5Ctext%7BVolume%7D_i%7D%7B%5Csum%20%5Ctext%7BVolume%7D_i%7D)

где:
- Price_i — цена на бирже i,
- Volume_i — объем торгов для этой цены на той же бирже. Это позволяет получить общую цену, в которой учтены объемы торгов на каждой платформе.
...
![Market Cap](https://latex.codecogs.com/png.latex?%5Ctext%7BMarket%20Cap%7D%20%3D%20%5Ctext%7BVWAP%7D%20%5Ctimes%20%5Ctext%7BCirculating%20Supply%7D)

где:
- Circulating Supply — актуальный объём криптовалюты в обращении. Этот параметр обычно берется из блокчейна или других надежных источников, а не бирж, так как он должен быть един и не зависит от торгов.
...
![Rating Formula](https://latex.codecogs.com/png.latex?%5Ctext%7BRating%7D_i%20%3D%20w_1%20%5Ccdot%20%5Ctext%7BMarket%20Cap%7D_i%20%2B%20w_2%20%5Ccdot%20%5Ctext%7BLiquidity%7D_i%20%2B%20w_3%20%5Ccdot%20%5Ctext%7BVolume%7D_i)

где:
- Rating — итоговый рейтинг криптовалюты i,
- Market Cap — нормализованная рыночная капитализация,
- Liquidity — нормализованная ликвидность,
- Volume — нормализованный объем торгов,
...
| Сервис | Функционал |
| ------ | ---------- |
| API gateway | Проксирует запросы в нужный микросервис в зависимости от URL запроса |
| auth service | Взаимодействует с данными пользователя. Отвечает за работу с сессиями пользователей, а также за такие функции, как регистрация и авторизация |
| prepare aleart service | Отвечает за подготовку списков пользователей, которым нужно отправить уведомления |
| aleart delivery service | Отвечает непосредственно за отправку уведомлений пользователям |
| real time price service | Отвечает за real-time отправку сообщений об изменении цены через websocket |
| data aggregation service | Отвечает за агрегирование данных из внешних источников |
| data request service | Совершает регулярные запросы в источники внешних данных наподобие бирж и т.п. с целью получения информации о ценах
| main service | Обеспечивает оставшуюся функциональность веб-приложения: отдаёт данные для рендеринга веб-страниц  |

### Фронтенд
**Мобильный клиент и веб-клиент:** Пользовательские интерфейсы для доступа к приложению. Мобильный клиент обращается к сервеной части веб-приложения через **Mobile Nginx**, а веб-клиент — через **Web Nginx**.
**Mobile Nginx и Web Nginx:** Проксируют и маршрутизируют запросы между клиентами и бэкендом.

### Бэкенд
**API Gateway:**
Принимает запросы от клиентов и маршрутизирует их в соответствующие микросервисы в зависимости от URL. Например, запросы на получение данных об аккаунте направляются в Auth Service, а запросы на получение информации о ценах — в Main Service.

**Auth Service:**
Управляет данными пользователей, авторизацией, регистрацией и сессиями. Поддерживает проверку прав доступа. Взаимодействует с БД Postgres и Redis.

**Main Service:**
Отвечает за основную функциональность веб-приложения. Например, обработку запросов на получение данных для рендеринга страниц или предоставление информации клиентам через API.

**Real Time Price Service:**
Отвечает за отправку TCP сообщений через WebSocket об изменении цен на активы в режиме реального времени через WebSocket. Это сервис содержит в себе открытые WebSocket-ы

**Prepare Alert Service:**
Формирует списки пользователей, которым нужно отправить уведомления, на основе событий (например, изменения цен). Списки передаются в Alert Delivery Service через очередь сообщений RabbitMQ.

**Alert Delivery Service:**
Отправляет уведомления пользователям, например, через push-уведомления и уведомления в браузере. Для балансировки нагрузки используется очередь сообщений RabbitMQ.

**Data Aggregation Service:**
Агрегирует данные, полученные из различных внешних источников, например, криптовалютных бирж. Эти данные могут включать цены, объемы торгов и другие метрики. Агрегированные данные заносятся в базу данных.

**Data Request Service:**
Регулярно запрашивает данные из внешних источников, таких как биржи, и передает их очередь RabbitMQ, откуда их потом забирает Data Aggregation Service. Работает в асинхронном режиме.

### Хранилища и мониторинг
**Postgres:** Используется для долговременного хранения данных, таких как информация о пользователях, история изменения цены, результаты агрегации и другие данные приложения.
**Redis:** Применяется для хранения сессий и json-подобных документов с данными для рендеринга графиков на клиенте.
**RabbitMQ:** Очередь сообщений, используемая для обмена данными между микросервисами (например, для передачи задач на отправку уведомлений).
**Prometheus и Grafana:** Мониторинг состояния системы. Prometheus собирает метрики с микросервисов, а Grafana визуализирует их и отправляет алерты в случае сбоев.

### Потоки данных
- Клиент отправляет запрос (например, для получения информации о ценах) через Nginx, который передает его в API Gateway.
- API Gateway направляет запрос в соответствующий микросервис (например, Main Service для данных о ценах). API Gateway не используется при отправке уведомлений на клиент и для отправки TCP сообщений через WebSocket.
- Для получения данных из внешних источников Data Request Service запрашивает данные и передает их в Data Aggregation Service, который на основании информации из множества источников выдаёт какое-то своё значение и сохраняет его в базе данных.
- Для уведомлений о достижении цены пороговых значений Prepare Alert Service формирует списки пользователей, котором необходимо отослать оповещения. А Alert Delivery Service асинхронно отправляет уведомления с использованием данных из очереди RabbitMQ, порционно забирая оттуда уведомления для отправки.
- Данные о состоянии системы собираются в Prometheus и отображаются в Grafana.


### Ключевые моменты
- внешние источники данных отдают временную метку, для которой актуальны отданные данные
- синхронизация по времени должна присутствовать только в рамках одной криптовалюты; синхронизация между несколькими криптовалютами не требуется
- так как на основе данных, циркулирующих внутри веб-сервиса, не проводится никаких транзакций по купле-продажи активов, то данные могут отличаться от актуальных, и информация может обновляться с задержкой
- уведомления в браузерном клиенте осуществляются через воркеры (workers). После закрытия вкладки воркер остается активным в фоновом режиме до тех пор, пока не будет явно остановлен браузером. Поэтому уведомление будет доставлено даже, если вкладка веб-приложения не активна или вообще закрыта.

# 11. Список серверов

#### Лёгкие запросы
Такие запросы обычно предполагают:

- Получение данных из базы без сложных вычислений.
- Минимальную обработку.

#### Средние запросы
Такие запросы обычно предполагают:

- Вычисления средней сложности (в нашем случае это авторизация).
- Многоуровневую обработку данных.


### Оценка ресурсов на 1000 RPS для backend на Go:
| Тип запроса  | CPU (ядра)  | RAM (Гб)  |  
|:-------:|:-----------:|:---------:|
| Лёгкий  |  ~1-2   	| ~0.2-0.4  | 
| Средний |  ~10        | ~2        | 


### Использование AWS (или другого облачного провайдера) вместо покупки собственных серверов
Использование AWS (или другого облачного провайдера) может быть более финансово целесообразным, чем покупка и поддержание собственных серверов для веб-приложения с нагрузкой, которая в пиковые моменты может достигать 100-кратного увеличения от средней (см. [2. Расчет нагрузки](#2-расчет-нагрузки)). Обоснование строится на следующих ключевых моментах:

#### 1. Капитальные затраты (CapEx) vs Операционные затраты (OpEx)

**Собственные серверы (CapEx):**
- Покупка серверов требует значительных первоначальных инвестиций.
- Нужно закладывать избыточные ресурсы, чтобы покрыть пиковые нагрузки, даже если такие нагрузки возникают редко.
- Оборудование устаревает, и его нужно заменять каждые 3–5 лет, что увеличивает долгосрочные расходы.

**AWS (OpEx):**
- Оплата идет только за фактически использованные ресурсы.
- Нет необходимости покупать или обслуживать оборудование — вычислительные мощности арендуются только тогда, когда они нужны.

Таким образом AWS позволяет избегать значительных затрат на оборудование, которое большую часть времени будет простаивать.

#### 2. Эффективность использования ресурсов
**Собственные серверы:**
- Большая часть оборудования простаивает, так как нагрузка значительно ниже в обычные моменты.
- Поддержание инфраструктуры для обеспечения пиковой нагрузки приводит к низкому коэффициенту использования ресурсов (например, 10–20%).

**AWS:**
- Ресурсы масштабируются динамически, добавляя мощность только в пиковые моменты.
- Во время среднего и низкого уровня нагрузки используется минимально необходимое количество ресурсов.

Если средняя нагрузка требует 10 серверов, а пиковая — 100 серверов, собственная инфраструктура вынуждена работать с 90 лишними серверами в непиковое время. В AWS просто отключаются лишние инстансы, когда нагрузка падает.

#### 3. Масштабируемость
**Собственные серверы:**
- Масштабирование требует дополнительных вложений:
- В пиковый момент, если не хватило ресурсов, придется закупать оборудование (что требует времени).
- Избыток оборудования в обычное время — это потеря денег.

**AWS:**
- Поддерживается автоматическое масштабирование. Ресурсы увеличиваются за минуты и уменьшаются, как только нагрузка спадает.
- Услуги, такие как EC2 Auto Scaling и Elastic Load Balancing, позволяют поддерживать производительность без избыточных затрат.

Если нагрузка на веб-приложение внезапно возрастает в 100 раз, в AWS достаточно настроить масштабирование, и новые инстансы EC2 автоматически добавятся в пул. Для собственных серверов потребовались бы месяцы планирования и закупок.
...
Сетевая нагрузка на сервера складывается из следующих составляющих. ## 2. Расчет нагрузки
Пиковый общий трафик (см. [2. Расчет нагрузки](#2-расчет-нагрузки)):
* посещаемые страницы: 3,01 Гбит/с.
* действия на страницы: 75 Мбит/с.
* websocket: 17,1 Гбит/с.
* общий пик: 20,185 Гбит/с.
...
Пиковое RPS (HTTP-запросы) (см. [2. Расчет нагрузки](#2-расчет-нагрузки)):
* посещаемые страницы: 8 800
* действия на страницы: 16 800
* websocket: 1 400 700
* общий пик (без учёта websocket): 25 600 
* общий пик (с учётом websocket): 1 426 300

Сколько серверов потребуется для обработки пикового трафика:
* 20,185 Гбит/с ÷ 100 Гбит/с/сервер ≈ 0.2 сервера.

Теоретически весь трафик может быть обработан одним сервером Nginx, но, конечно, так делает не следует. И нужно сделать по одному серверу Nginx в каждом датацентре.
И так как всего будет 18 ЦОДов, то будет и 

Для SSL-терминации требуется учитывать не только объем соединений, но и RPS. Один сервер может обрабатывать до 50,000 RPS с SSL:
* 1 426 300 RPS ÷ 50,000 RPS/сервер ≈ 28 серверов.

SSL-терминация становится ключевым фактором, поэтому потребуется 50 серверов только для обработки RPS. Таким образом, несмотря на низкую сетевую нагрузку, итоговое число серверов прищлось увеличить до 28.

Redis используется для хранения session tickets после SSL-терминации. Один session ticket занимает ~1 КБ. Для 1 426 300 RPS и сессии длительностью 10 секунд потребуется:
* 1 426 300 tickets × 1 КБ ≈ 1.4 ГБ памяти в Redis
...
Таким образом, количество серверов: 28 серверов для обработки пикового трафика и SSL-терминации.
...
### PostgreSQL
Объём всех таблиц составляет ~ 1.25 ТБ.
...
Каждый узел (соотношение ~ 1 к 20 между RAM и объёму данных):
* CPU: 16 vCPU.
...
|--------|--------------|-----|--------|
| main service | 25 600  | 25 | 13     | 
| auth service | 100 | 1 | 0.2 | 
| real time price service | 1 400 700 | 143 | 285 | 


* data aggregation service (не связан с запросами пользователей, не связан с внешними факторами и не требует динамического масштабирования)
* prepare alert service (не связан с запросами пользователей, но связан с внешними факторами, обусловленными рыночной ситуацией)

| Ресурс     | Итоговое количество | Стоимость аренды в месяц |
|------------|---------------------|--------------------------|
| vCPU       |    216              |  ~12 000$ ( всё вместе ) |
| RAM (ГБ)   |    682              |                          |
| SSD (ГБ)   |    6 000            |                          |
