...
- Минимальную обработку (например, фильтрация, сортировка, формирование JSON).
- Малый объем ответов (до 10-100 КБ).

#### Средние запросы
Такие запросы обычно предполагают:

- Вычисления средней сложности (например, агрегация данных, расчеты).
- Многоуровневую обработку данных (получение из нескольких таблиц или вызов API).
- Объем ответа: до 500 КБ.
...
|---|---|---|---|---|
| main service | 1 010  | 20 | 4 | 
| auth service | 953 | 10 | 2 | 
| real time price service | 1 426 300 | 143 | 285 | 


data aggregation service (не связан с запросами пользователей и не требует динамического масштабирования)
